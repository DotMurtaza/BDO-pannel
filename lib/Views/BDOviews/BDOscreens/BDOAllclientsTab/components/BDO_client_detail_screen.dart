import 'package:bdo_pannel/Views/BDOviews/BDOwidgets/BDO_details_info_row.dart';
import 'package:bdo_pannel/models/BDO_clients_model.dart';
import 'package:bdo_pannel/models/client_model.dart';
import 'package:flutter/material.dart';

import '../../../../../constants.dart';

class BDOclientDescription extends StatelessWidget {
  const BDOclientDescription({
    Key key,
    this.info,
    this.clientModel,
  }) : super(key: key);
  final BDOclientModels info;
  final ClientModel clientModel;

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        elevation: 0,
        backgroundColor: kPrimaryColor,
        title: Text(''),
      ),
      body: Padding(
        padding: const EdgeInsets.all(kPadding),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(
              "Client Info",
              style: Theme.of(context).textTheme.headline6,
            ),
            BDOinfoRow(
              label: "Name",
              value: clientModel.clientName,
            ),
            BDOinfoRow(
              label: "Mobile No.",
              value: clientModel.clientContact.toString(),
            ),
            BDOinfoRow(
              label: "Email",
              value: clientModel.clientEmail,
            ),
            BDOinfoRow(
              label: "City",
              value: clientModel.clientCity,
            ),
            const SizedBox(height: kPadding * 1.5),
            Text(
              "Project Info",
              style: Theme.of(context).textTheme.headline6,
            ),
            BDOinfoRow(
                label: 'Business Name', value: clientModel.clientBusiness),
            // BDOinfoRow(
            //     label: 'Client Contact', value: '${clientModel.clientContact}'),
            BDOinfoRow(
                label: 'Date/Time',
                value: clientModel.time.toDate().toString()),
            // BDOinfoRow(
            //     label: 'Discription', value: '${clientModel.discription}'),
            SizedBox(
              height: kPadding,
            ),
            Align(
              alignment: Alignment.centerLeft,
              child: Text(
                "Description:",
                style: Theme.of(context)
                    .textTheme
                    .bodyText2
                    .copyWith(color: Colors.grey[600]),
              ),
            ),
            SizedBox(height: kPadding * 0.25),
            Text(
              clientModel.discription,
              style: Theme.of(context).textTheme.subtitle1,
            ),
          ],
        ),
      ),
    );
  }
}
